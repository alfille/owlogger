...
class Connection(object):
    ...
    def __init__(self, server, port):
        ...
        """
        Dprint( f"Connection.__init__({server}, {port})" )
    ...
    def __repr__(self):
        ...
        Dprint( 'Connection.__repr__')
        return f"Connection(\"{self._server}\", {self._port})"
    ...
    def write(self, path, value):
        ...
        Dprint( f"Connection.write(\"{path}\", \"{str(value)}\")" )
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.connect((self._server, self._port))
    ...
    def pack(self, function, payload_len, data_len):
        ...
        Dprint( f"Connection.pack({function}, {payload_len}, {data_len})" )
    ...
    def unpack(self, msg):
        ...
        Dprint( f"Connection.unpack(\"{msg}\")" )
        if len(msg) != 24:
