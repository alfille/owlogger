...
# apt update
# apt upgrade
# apt install python3 python3-jwt python3-bcrypt git ufw caddy gunicorn python3-pil python3-flask

echo
echo "---------------------------------"
echo "OWLOGGER-GUNICORN install process starting"
...
# systemd file
cp -i owloggerf.service  /etc/systemd/system
systemctl daemon-reexec
systemctl daemon-reload
systemctl enable owloggerf.service
systemctl restart  owloggerf.service
systemctl status --no-pager owloggerf.service

echo "OWLOGGER-GUNICORN install process finished"
echo "---------------------------------"
echo
