...
class ShortWrite(ProtocolError):
    ...
    def __str__(self):
        return f"sent {self.sent} bytes instead of {self.tosend}."
...
class OwnetTimeout(Error):
    ...
    def __str__(self):
        return f"Communication with owserver aborted after {self.elapsed:.1f}s, timeout of {self.timeout:.1f}s exceeded"
...
class _Header(bytes, metaclass=_addfieldprops):
    ...
    def _parse(cls, *args, **kwargs):
        if args:
            ...
            if kwargs:
                raise TypeError(f"constructor got unexpected keyword argument '{kwargs.popitem()[0]}'")
...
class _OwnetConnection:
    ...
    def __str__(self):
        return f"_OwnetConnection {self.socket.getsockname()} -> {self.peername}"
    ...
    def shutdown(self):
        ...
        try:
            ...
            assert err.errno is _ENOTCONN, f"unexpected IOError: {err}"
...
class _Proxy(object):
    ...
    def __str__(self):
        return f"owserver at {self._sockaddr}"
